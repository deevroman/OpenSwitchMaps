	...
	*/
		...
		getLatLonZoom(url) {
			...
			if (match) {
				...
				if (u.search && u.searchParams.has("pin")) {
					const pin = decodeURIComponent(u.searchParams.get("pin"));
...

	{
		...
		getLatLonZoom(url) {
			let lat,
				lon,
				zoom = 16; // 16 as default zoom level
	...
	},
	{
		...
		getUrl(lat, lon, zoom) {
			return `https://api.panoramax.xyz/#focus=map&map=${zoom}/${Number(lat).toFixed(5)}/${Number(lon).toFixed(5)}&speed=250`;
		},
		getLatLonZoom(url) {
